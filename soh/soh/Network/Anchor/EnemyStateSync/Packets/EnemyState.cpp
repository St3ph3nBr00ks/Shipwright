...
void Anchor::SendPacket_EnemyUpdate(uint32_t netId, Actor* actor) {
    ...
    // only state changes for the actors we're actively debugging.
    {
        ...
        if (extras.hasEnFirefly) {
            s16 prev = prevExtras && prevExtras->hasEnFirefly ? prevExtras->enFireflyActionState : -1;
            if (prev != extras.enFireflyActionState) {
                SPDLOG_INFO("[EnFirefly] tx netId={} state={}→{}", netId,
...
void Anchor::HandlePacket_EnemyDefeated(nlohmann::json payload) {
    ...
    // (Karebaba→MISC, Armos→BG, etc.).
    {
        ...
        if (actor != nullptr) {
                ...
                // En_Firefly (Keese): route through EnFirefly_SetupDyingNet so
                // the Fall -> SetupDie -> Die natural cycle plays on the receiver.
                // The natural SetupDie call DOES fire OnEnemyDefeat, but the
                // HostBookkeeping HasDefeatBroadcast dedup guard prevents
                // echo to host. Plan section 4 step 6 / #47.
                if (actor->id == ACTOR_EN_FIREFLY) {
                    EnemyStateSync::AuditBooleansVsPhase(*ext, "HandlePacket_EnemyDefeated.EnFirefly.dupDetect");
                    if (EnemyStateSync::PhaseImpliesHasLocalDeath(ext->phase)) {
                        SPDLOG_INFO("[EnemyDefeated] EnFirefly netId={} already dying — duplicate, dedup only", netId);
                        EnemyStateSync::HostBookkeeping::Instance().RecordPendingKill(netId);
                        return;
                    }
