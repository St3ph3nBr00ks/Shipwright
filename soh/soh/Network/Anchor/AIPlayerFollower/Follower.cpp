    ...
    auto TeleportToNextSubgoal = [&](const char* reason) -> bool {
        ...
        if (isClimbDest && navData != nullptr) {
            ...
            if (resolvedAnchor != UINT16_MAX) {
                ...
                followerStateFrames           = 0;
                followerClimbStuckCheckPos    = player->actor.world.pos;
...
void Anchor::HandleStateClimbing(Player* player, const Vec3f& leaderPos, Actor* leaderActor) {
    ...
    // R1 of follower_nav_refactor: dispatch to one of two phase helpers
    // based on followerAutonomousClimb. Both phases share the dismount-
    // yaw + state-cleanup helpers (ComputeFollowerDismountYaw,
    // ExitFollowerClimbToIdle) so the legacy duplicate `computeDismountYaw`
    // lambda + 6-line reset boilerplate are gone.
    if (followerAutonomousClimb) {
        HandleClimbStateAutonomous(player, leaderPos);
    } else {
        HandleClimbStateLeaderFollowing(player, leaderPos, leaderActor);
    }
}

s16 Anchor::ComputeFollowerDismountYaw(Player* player) const {
    // Dismount yaw points from anchor.topPos toward the nearest floor
    // node — the boundary edge by construction (Stage 3 per-cell
    // nearest-floor lookup). This IS the platform Link should walk
    // onto after climbing.
    //
    // Why not use leader pos: leader can move during the climb;
    // by the time follower reaches the top, leader may have walked
    // to a position that doesn't correspond to the climb-top platform.
    //
    // Why not use anchor.planeNormal: for Path A ladder actors,
    // planeNormal = actor's shape.rot.y direction, which often points
    // INTO the wall (ladder actors face the wall they lean against),
    // not away from it. Direction-from-topPos-to-nearest-floor doesn't
    // have this geometry ambiguity.
    //
    // Falls back to shape.rot.y if no nearby floor is found (rare —
    // implies the climb top is over a void).
    if (followerClimbAnchorIdx != UINT16_MAX) {
        const ::AnchorNavRoom::RoomNavData* navData =
            ::AnchorNavRoom::GetForRoom(
                gPlayState->sceneNum,
...
void Anchor::ExitFollowerClimbToIdle(s16 dismountYaw, bool clearAutonomous) {
...
void Anchor::HandleClimbStateAutonomous(Player* player, const Vec3f& leaderPos) {
    ...
    // fix; refined log 100 followup). When the follower is stuck in
    // CLIMBING and would normally exit to IDLE, attempt to advance
    // the path cursor and teleport forward.
    //
    // Two-pass priority order (per user preference: stay on the
    // wall whenever possible):
    //
    //   Pass 1: scan ahead for the next CLIMB cell. If found,
    //           teleport there, STAY in CLIMBING — Link's collision
    //           should re-attach to the wall (cell.pos is on the
    //           wall surface). Reset the stuck-check baseline so
    //           progress resumes from the new position. Cursor
    //           advances each time, so repeated stucks naturally
    //           walk the cursor up the climb segment.
    //
    //   Pass 2: no climb cells ahead within scan window — escalate
    //           to next NON-CLIMB waypoint (drop or floor; natural
    //           climb-segment exit). Teleport there, exit CLIMBING
    //           → FOLLOW so the next tick picks up the drop/walk
    //           transition immediately.
    //
    // If neither pass finds a target (path empty or cursor at end),
    // returns false — caller falls back to legacy IDLE exit, and
    // G14 escalates from there.
    auto teleportForwardOnClimbStuck = [&](const char* reason) -> bool {
        ...
        // Pass 1: prefer next CLIMB cell (continue along climb segment).
        for (size_t i = cur + 1; i < scanEnd; i++) {
            const uint32_t f = (i < followerNavPath.waypointFlags.size())
                                   ? followerNavPath.waypointFlags[i]
                                   : (uint32_t)0;
            if ((f & ::AnchorNavRoom::NODE_CLIMB_ANY) != 0) {
                const Vec3f& target = followerNavPath.waypoints[i];
                SPDLOG_INFO("[Follower] CLIMBING teleport-to-next-climb ({}) "
                            "— cursor {} -> {}/{} pos=({:.0f},{:.0f},{:.0f})",
                            reason, (int)cur, (int)i,
                            (int)followerNavPath.waypoints.size() - 1,
                            target.x, target.y, target.z);
                player->actor.world.pos = target;
                followerNavPath.cursorIdx = i;
                followerPostTeleportFrames = kPostTeleportHoldFrames;
                // Reset stuck baseline at new position — fresh 2s
                // window before recovery C can fire again.
                followerClimbStuckCheckPos = target;
                followerAutonomousClimbFrames = 0;
                // STAY in CLIMBING state. Link's body is at the
                // wall surface (cell.pos was a raycast hit on the
                // wall); OoT's vine/ladder collision should
                // re-attach him on the next physics tick.
                return true;
            }
        }

        // Pass 2: no climb cells ahead — escalate to next non-climb
        // (drop / floor exit). Same teleport-and-cleanup as the
        // pre-refactor logic.
        for (size_t i = cur + 1; i < scanEnd; i++) {
            const uint32_t f = (i < followerNavPath.waypointFlags.size())
                                   ? followerNavPath.waypointFlags[i]
            ...
            if ((f & ::AnchorNavRoom::NODE_CLIMB_ANY) == 0) {
                const Vec3f& target = followerNavPath.waypoints[i];
                SPDLOG_INFO("[Follower] CLIMBING→FOLLOW teleport-to-non-climb "
                            "({}) — cursor {} -> {}/{} pos=({:.0f},{:.0f},{:.0f})",
                            reason, (int)cur, (int)i,
                            (int)followerNavPath.waypoints.size() - 1,
                            target.x, target.y, target.z);
    ...
    // autonomous climb, compare current 3D pos to the last-checkpoint
    // pos. If 3D displacement < kClimbStuckMinProgress (20u), the
    // follower has been sitting in CLIMBING without making meaningful
    // progress (e.g. stuck in autonomous climb against an unreachable
...
void Anchor::HandleClimbStateLeaderFollowing(Player* player, const Vec3f& leaderPos, Actor* leaderActor) {
    ...
    if (it == clients.end() || !it->second.isClimbing) {
        ...
        // See ComputeFollowerDismountYaw for rationale (leader pos
        // unreliable; planeNormal ambiguous for ladder actors).
        const s16 dismountYaw = ComputeFollowerDismountYaw(player);
        ExitFollowerClimbToIdle(dismountYaw, /*clearAutonomous=*/false);
    ...
    if (climbBits != 0 &&
        ...
        if (followerClimbAnchorIdx != UINT16_MAX && navData != nullptr) {
            ...
            if (descentIntent && atOrAboveAnchorTop) {
                ...
                // Arm post-teleport hold so the next few frames of stick
                // injection don't fight vanilla's pose-settle. The
