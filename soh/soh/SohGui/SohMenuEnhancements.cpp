...
void SohMenu::AddMenuEnhancements() {
    ...
    AddWidget(path, "Jump Anchor Detection (pre-baked traversal pairs)", WIDGET_CVAR_CHECKBOX)
        ...
        .RaceDisable(false)
        .Options(CheckboxOptions().Tooltip(
            "When on, scans for walkable-node pairs separated by a true "
            "air gap (no continuous floor along the segment between them). "
            "Same-altitude jumps and upward jumps (up to broad-jump apex) "
            "are pre-baked into NavEdges so BFS can route across without "
            ...
            "XZ range: 18-150u. Y range: -30u to +80u (matching upward "
