...
static Player* EnSw_PickLungeTarget(EnSw* this, PlayState* play, s32 arg2) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        // PLAYER_STATE2_STATIONARY_LADDER, set every frame the player
        // is on a ladder/vine with no stick input).
        //
        // Log 446 cause: pre-fix this called `func_8002DDF4(play)`
        // which reads GET_PLAYER's stateFlags2 — i.e., the LOCAL
        // Link's flag, never the iterated candidate's. On host, P1's
        // stationary state gated En_Sw for every player including P2.
        ...
        // subtraction + a normalise-to-[-0x8000, 0x8000) step so the
        // shortest angular distance is computed without the s16 wrap
        // that would otherwise false-pass for angular deltas > 180°
        // (latent vanilla bug; rarely triggered in single-player
