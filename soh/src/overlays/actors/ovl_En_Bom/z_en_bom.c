...
void EnBom_Init(Actor* thisx, PlayState* play) {
    ...
    if (Anchor_GetEnforcedFloat(CVAR_CHEAT("BombTimerMultiplier"), 1.0f) != 1.0f) {
        this->timer = (s32)(70 * Anchor_GetEnforcedFloat(CVAR_CHEAT("BombTimerMultiplier"), 1.0f));
...
void EnBom_Update(Actor* thisx, PlayState* play2) {
    ...
    if (this->timer == 67 && !GameInteractor_GetRandomBombFuseTimerActive() &&
        Anchor_GetEnforcedFloat(CVAR_CHEAT("BombTimerMultiplier"), 1.0f) == 1.0f) {
        Audio_PlayActorSound2(thisx, NA_SE_PL_TAKE_OUT_SHIELD);
        Actor_SetScale(thisx, 0.01f);
    }
    ...
    if (thisx->params == BOMB_BODY) {
        float timerMultiplier = Anchor_GetEnforcedFloat(CVAR_CHEAT("BombTimerMultiplier"), 1.0f);
